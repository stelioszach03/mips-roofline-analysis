# Πολλαπλασιασμός διανύσματος με βαθμωτό (n=8)
# Υλοποίηση για QtMips με βασικές εντολές

.data
    # Διάνυσμα εισόδου (Input vector)
    vector: .word 1, 2, 3, 4, 5, 6, 7, 8
    # Βαθμωτός αριθμός (Scalar value)
    scalar: .word 2
    # Διάνυσμα αποτελέσματος (Result vector)
...
store_result:
    addiu $7, $6, 36       # Διεύθυνση αποτελέσματος
